            ...
            for (int r=board.size()-2; r>=0; r--) {
                ...
                if (t != null) {
                    ...
                    for (int i=board.size()-1; i>r; i--) {
                        if (merged.contains(board.tile(c, i))) {
                            continue;
                        }
                        Tile curT = board.tile(c, i);
                        if (curT == null && !valueExist) {
                            targetRow = i;
                            valueExist = true;
                        } else if (curT != null && curT.value() == t.value()) {
                            targetRow = i;
                            valueExist = true;
                        } else if (curT != null && curT.value() != t.value()) {
                            targetRow -= 1;
                    ...
                    boolean ifMerge = board.move(c, targetRow, t);
                    changed = true;

                    if (ifMerge) {
                        score += t.value()*2;
                        merged.add(board.tile(c, targetRow));
                    }
